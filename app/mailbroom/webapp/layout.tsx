...
// for inbound lead surfacing) — scoped to MailBroom for Business pages
// only, not the whole aiert-website (ShareQuest, main AIERT site, etc.),
// since this is B2B-buyer traffic (the actual Apollo ICP), unlike
// aiert.co.uk (fintech) or ios.mailbroom.app (consumer iOS app). Originally
// meant for business.mailbroom.app, but that host now 301s straight to
// mailbroom.app (see next.config.ts) before a page ever loads, so it
// belongs here instead — this layout is what actually renders for
// mailbroom.app/webapp/* traffic.
//
...
export default function MailBroomWebAppLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      ...
      />
      {/* Lemon Squeezy affiliate tracking — must live on business.mailbroom.app
      since that's the store's configured "Affiliate referral URL" (Settings ->
      Affiliates); a visitor arriving here via an affiliate link gets the
      referral cookie set here, which Lemon Squeezy's checkout later reads to
      attribute the sale. */}
      <script
        dangerouslySetInnerHTML={{ __html: `window.lemonSqueezyAffiliateConfig = { store: "mailbroom" };` }}
      />
